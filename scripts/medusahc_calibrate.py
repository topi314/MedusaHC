...
class MedusaHCCalibrate:
    ...
    def cmd_TOOL_CALIBRATE_TOOL_OFFSET(self, gcmd):
        ...
        if gcmd.get_int("APPLY", 0):
            ...
            if medusa is None:
                raise gcmd.error("APPLY=1: medusahc not loaded")
            ct = int(medusa.current_tool)
            if ct < 0:
                raise gcmd.error("APPLY=1: no tool on head")
            x = float(self.last_result[0])
            y = float(self.last_result[1])
            z = float(self.last_result[2])
            medusa.set_offset(ct, x, y, z)
            self.gcode.respond_info(
                "APPLY: TOOL_OFFSET T%d <- %.6f,%.6f,%.6f" % (ct, x, y, z)
            )
    ...
    def _parse_tools(self, gcmd, medusa):
        ...
        tool_count = int(medusa.tool_count)
        if raw is None:
            return [t for t in range(1, tool_count)]
    ...
    def cmd_CALIBRATE_TOOL_OFFSETS(self, gcmd):
        ...
        for t in tools:
            ...
            self._respond("CALIBRATE_APPLY_OFFSET: T%d X=%.6f Y=%.6f Z=%.6f" % (t, x, y, z))
        ...
        if gcmd.get("SAVE", None) is None and gcmd.get("DROP", None) is None:
            self.cmd_CALIBRATE_TOOL_OFFSETS(
                self.gcode.create_gcode_command(
                    "CALIBRATE_TOOL_OFFSETS", "CALIBRATE_TOOL_OFFSETS",
                    {"SAVE": "1", "DROP": "1"}))
            return

        save = int(gcmd.get_int("SAVE", 1))
        drop = int(gcmd.get_int("DROP", 1))
        params = {"SAVE": str(save), "DROP": str(drop)}
        tools = gcmd.get("TOOLS", None)
        if tools is not None:
            params["TOOLS"] = tools
        self.cmd_CALIBRATE_TOOL_OFFSETS(
            self.gcode.create_gcode_command(
                "CALIBRATE_TOOL_OFFSETS", "CALIBRATE_TOOL_OFFSETS", params))

    def cmd_CALIBRATE_NOZZLE_PROBE_OFFSET(self, gcmd):
    ...
    def cmd_SAVE_TOOL_GCODE_OFFSETS(self, gcmd):
        ...
        tool_count = int(medusa.tool_count)
        if t < 0 or t >= tool_count:
            raise gcmd.error("SAVE_TOOL_GCODE_OFFSETS: bad T")
